...
def _tensor_array_concat(handle, flow_in, dtype, name=None):
  r"""Concat the elements from the TensorArray.
  ...
  Returns:
    ...
    lengths: A `Tensor` of type `int64`. A vector of the row sizes of the original T elements in the
      ...
      (n1, n2, ..., n(T-1))
  """
  return _op_def_lib.apply_op("TensorArrayConcat", handle=handle,
...
def _tensor_array_split(handle, value, lengths, flow_in, name=None):
  ...
  Returns:
    ...
    A float scalar that enforces proper chaining of operations.
  """
  return _op_def_lib.apply_op("TensorArraySplit", handle=handle, value=value,
                              lengths=lengths, flow_in=flow_in, name=name)
